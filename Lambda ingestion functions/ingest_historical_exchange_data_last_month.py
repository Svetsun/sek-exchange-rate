...
def lambda_handler(event, context):
    ...
    # Calculate the first day of the month, six months prior to the current month
    first_day_of_current_month = end_date.replace(day=1)
    first_day_of_six_months_ago = (first_day_of_current_month - datetime.timedelta(days=1)).replace(day=1)
    for _ in range(5):
        first_day_of_six_months_ago = (first_day_of_six_months_ago - datetime.timedelta(days=1)).replace(day=1)
    
    start_date = first_day_of_six_months_ago
